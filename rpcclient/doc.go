...
style Decred JSON-RPC API.  This client has been tested with hxd2
(https://github.com/hunjixin/hxd2) and dcrwallet
(https://github.com/hunjixin/dcrwallet).

In addition to the compatible standard HTTP POST JSON-RPC API, hxd2 and
...
hxd2 or dcrwallet by default.  However, configuration options are provided to
...
In contrast, the websocket-based JSON-RPC interface provided by hxd2 and
...
All notifications provided by hxd2 require registration to opt-in.  For example,
...
example, the DebugLevel call is an extension only provided by hxd2 (and
...
Also, it is important to realize that hxd2 intentionally separates the wallet
functionality into a separate process named dcrwallet.  This means if you are
connected to the hxd2 RPC server directly, only the RPCs which are related to
