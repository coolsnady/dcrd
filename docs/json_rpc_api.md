...
hxd2 provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API that is
fully compatible with the original bitcoind/bitcoin-qt.  There are a few key
differences between hxd2 and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  which leads to several issues, hxd2 intentionally splits the wallet and chain
  ...
  talking directly to hxd2, only chain-related RPCs are available.  However both
  chain-related and wallet-related RPCs are available via
  [dcrwallet](https://github.com/hunjixin/dcrwallet).
* hxd2 is secure by default which means that the RPC connection is TLS-enabled
  by default
* hxd2 provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for hxd2 RPC and are used by applications
such as [dcrwallet](https://github.com/hunjixin/dcrwallet) for inter-process
communication with hxd2.  The websocket connection endpoint for hxd2 is
...
The hxd2 RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
hxd2 is `wss://your_ip_or_domain:9109/ws`.
...
to a hxd2 RPC server:

* **rpcuser** is the full-access username configured for the hxd2 RPC server
* **rpcpass** is the full-access password configured for the hxd2 RPC server
* **rpclimituser** is the limited username configured for the hxd2 RPC server
* **rpclimitpass** is the limited password configured for the hxd2 RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the hxd2
  server is configured with.  It is automatically generated by hxd2 and placed
  in the hxd2 home directory (which is typically `%LOCALAPPDATA%\hxd2` on
  Windows and `~/.hxd2` on POSIX-like OSes)

**NOTE:** As mentioned above, hxd2 is secure by default which means the RPC
...
The hxd2 RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
hxd2 comes with a separate utility named `dcrctl` which can be used to issue
these RPC commands via HTTP POST requests to hxd2 after configuring it with the
...
API compatible with the original hxd2 client.
...
|24|[getwork](#getwork)|N|Returns formatted hash data to work on or checks and submits solved data.<br /><br />NOTE: Since hxd2 does not have the wallet integrated to provide payment addresses, hxd2 must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
...
|27|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><br />NOTE: hxd2 does not yet implement the `allowhighfees` parameter, so it has no effect|
|28|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since hxd2 does not have the wallet integrated to provide payment addresses, hxd2 must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|29|[stop](#stop)|N|Shutdown hxd2.|
|30|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|31|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since hxd2 does not have a wallet integrated, hxd2 will only return whether the address is valid or not.|
...
|35|[getcurrentnet](#getcurrentnet)|Y|Get Decred network hxd2 is running on.|
...
|Notes|NOTE: Since hxd2 does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in dcrwallet for a version which includes that information.|
...
|Example Return|`[{"addr": "178.172.xxx.xxx:9108", "services": "00000001", "lastrecv": 1388183523, "lastsend": 1388185470, "bytessent": 287592965, "bytesrecv": 780340, "conntime": 1388182973, "pingtime": 405551, "pingwait": 183023, "version": 70001, "subver": "/hxd2:0.4.0/", "inbound": false, "startingheight": 276921, "currentheight": 276955, "syncnode": true }, ...]`|
[Return to Overview](#MethodOverview)<br />
...
|Description|Returns information about a transaction given its hash.|
|Notes|Since hxd2 does not have the wallet integrated to provide payment addresses, hxd2 must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.
...
|Notes|Since hxd2 does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.|
...
|Notes|NOTE: Since hxd2 does not have the wallet integrated to provide payment addresses, hxd2 must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|hxd2 does not yet implement the `allowhighfees` parameter, so it has no effect.|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown hxd2.|
|Returns|`"hxd2 stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database. The actual checks performed by the `checklevel` parameter is implementation specific. <br /><br />For hxd2 this is: <br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|hxd2 currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for hxd2.|
...
|Description|Dynamically changes the debug logging level.<br />The levelspec can either a debug level or of the form `<subsystem>=<level>,<subsystem2>=<level2>,...`<br />The valid debug levels are `trace`, `debug`, `info`, `warn`, `error`, and `critical`.<br />The valid subsystems are `AMGR`, `ADXR`, `BCDB`, `BMGR`, `hxd2`, `CHAN`, `DISC`, `PEER`, `RPCS`, `SCRP`, `SRVR`, and `TXMP`.<br />Additionally, the special keyword `show` can be used to get a list of the available subsystems.|
|Returns|`string`|
|Example Return|`Done.`|
|Example `show` Return|`Supported subsystems [AMGR ADXR BCDB BMGR hxd2 CHAN DISC PEER RPCS SCRP SRVR TXMP]`|
...
|Parameters|None|
|Description|Get Decred network hxd2 is running on.|
|Returns|`numeric`|
...
|Description|Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this hxd2 instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSMethodOverview)<br />
...
hxd2 uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll hxd2 for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
[rpcclient](https://github.com/hunjixin/hxd2/tree/master/rpcclient) package.
...
[rpcclient](https://github.com/hunjixin/hxd2/tree/master/rpcclient) package to connect with
a hxd2 instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by hxd2 when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/hunjixin/hxd2/tree/master/rpcclient) package to connect with
a hxd2 instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/hunjixin/hxd2/tree/master/rpcclient) package to connect with
a hxd2 instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a hxd2 instance,
...
// generated by hxd2 when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/hxd2/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The hxd2 generated certificate acts as
