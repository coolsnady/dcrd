...
// hxd2Main is the real main function for hxd2.  It is necessary to work around
// the fact that deferred functions do not run when os.Exit() is called.  The
// optional serverChan parameter is mainly used by the service code to be
// notified with the server once it is setup so it can gracefully stop it when
// requested from the service control manager.
func hxd2Main(serverChan chan<- *server) error {
	...
	defer hxd2Log.Info("Shutdown complete")

	// Show version and home dir at startup.
	hxd2Log.Infof("Version %s (Go version %s)", version(), runtime.Version())
	hxd2Log.Infof("Home dir: %s", cfg.HomeDir)
	if cfg.NoFileLogging {
		hxd2Log.Info("File logging disabled")
	}
	...
	if cfg.Profile != "" {
		go func() {
			...
			hxd2Log.Infof("Creating profiling server "+
	...
	if cfg.CPUProfile != "" {
		...
		if err != nil {
			hxd2Log.Errorf("Unable to create cpu profile: %v", err.Error())
			return err
		}
	...
	if cfg.MemProfile != "" {
		...
		if err != nil {
			hxd2Log.Errorf("Unable to create mem profile: %v", err)
			return err
		}
	...
	db, err := loadBlockDB()
	if err != nil {
		hxd2Log.Errorf("%v", err)
		return err
	}
	defer func() {
		// Ensure the database is sync'd and closed on shutdown.
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventDBOpen)
		hxd2Log.Infof("Gracefully shutting down the database...")
		db.Close()
	}()
	...
	if err != nil {
		...
		hxd2Log.Errorf("Unable to start server on %v: %v",
			cfg.Listeners, err)
		return err
	}
	defer func() {
		lifetimeNotifier.notifyShutdownEvent(lifetimeEventP2PServer)
		hxd2Log.Infof("Gracefully shutting down the server...")
		server.Stop()
