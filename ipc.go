...
// processes to communicate with and and manage the lifetime of a hxd2 child
...
// message, shutdown begins.  This prevents hxd2 from continuing to run
...
func serviceControlPipeRx(fd uintptr) {
	...
	for {
		...
		if err != nil {
			hxd2Log.Errorf("Failed to read from pipe: %v", err)
			break
		}
	}
...
// for a child hxd2 process to communicate with a parent process without the
...
func serviceControlPipeTx(fd uintptr) {
	...
	hxd2Log.Errorf("Failed to write to pipe: %v", err)
}
