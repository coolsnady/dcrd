...
// arguments returns an array of arguments that be used to launch the hxd2
// process.
...
// command returns the exec.Cmd which will be used to start the hxd2 process.
...
// to the hxd2 process that is launched via Start().
...
// as the base for the log and data directories for hxd2.
...
// start creates a new hxd2 process, and writes its pid in a file reserved for
...
func (n *node) start() error {
	...
	pid, err := os.Create(filepath.Join(n.config.String(), "hxd2.pid"))
	if err != nil {
		return err
	}
...
// stop interrupts the running hxd2 process process, and waits until it exits
...
// shutdown terminates the running hxd2 process, and cleans up all
